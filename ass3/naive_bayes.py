...
Pendigits data: classification accuracy = 0.8228
Satellite data: classification accuracy = 0.7930
Yeast data: classification accuracy = 0.3430

'''

# preprocessing

y_train = pd.read_csv(sys.argv[1], header = None, delim_whitespace= True).to_numpy().astype(float)
...
ytr = y_train[y_train[:,y_train.shape[1]-1].argsort()] 
train_label = ytr[:,-1]
train_feat = ytr[:,0:-1]

y_test = pd.read_csv(sys.argv[2], header = None, delim_whitespace= True).to_numpy().astype(float)
...
yts = y_test[y_test[:,y_test.shape[1]-1].argsort()] 
test_label = yts[:,-1]
test_feat = yts[:,0:-1]
...
for i in range(counts.shape[0]):
    ...
    for j in range(label_idx):
        ...
        print("Class ", "%d" %class_n[i], ", attribute ", "%d" %int(j+1), ", mean = ", "%.2f" % train_means[i,j], ", std = ", "%.2f" % train_stdevs[i,j])
    tally += n_samples
    #print(tally)
    print()
...
def get_maxvals(t_cl, maxval):
    ...
    for i in range(np.argmax(t_cl), t_cl.shape[0]):
        if t_cl[i] == maxval:
...
for i, x_i in enumerate(yts):
    ...
    print("ID = ", "%5d" % int(i+1), ", predicted = ", "%3d" % prediction, ", probability = ", "%.4f" % tmax, ", true = ", "%3d" % test_label[i], ", accuracy = ", "%4.2f" % accurs[i])
