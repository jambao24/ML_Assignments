...
class NN_Manager:
    ...
    def __init__(self, learning_rate, n_iters, n_inputs, num_hidden, momentum):
        ...
        # NeuralNet parameters- learning rate, corresponding value from target array [digits 0-9], weights for inputs, weight for bias
        #self.h0 = NeuralNet(self.lr, -69, self.hidden_m_weights[0, 1:], self.hidden_m_weights[0,0])
        #self.h1 = NeuralNet(self.lr, -69, self.hidden_m_weights[1, 1:], self.hidden_m_weights[1,0])
    ...
    def __iterate__(self, train_X, train_Y, valid_X, valid_Y, acc_train, acc_valid):
        ...
        for i in range(self.n_it):
            ...
            # re-initialize weights in hidden and output neurons to updated values
            self.hidden_s = [HiddenNeur(self.lr, self.alpha, i, self.hidden_mw[i,1:], self.hidden_mw[i,0]) for i in range(self.num)]
            self.output_s = [OuterNeur(self.lr, self.alpha, i, self.output_mw[i,1:], self.output_mw[i,0]) for i in range(10)]
            h_t = np.asarray(self.hidden_s)
            o_t = np.asarray(self.output_s)
            self.hidden_s = h_t
            self.output_s = o_t
    ...
    def _iterate_helper_(self, data, target):
        ...
        for idx, x_i in enumerate(data):
            ...
            output_dot = np.dot(self.output_mw_cross, self.hidden_nodes) + self.output_mw_bias
            ...
            h_diffsum = np.dot(self.output_mw_cross.T, self.output_error)
            self.hidden_error = np.multiply(self.hidden_nodes, h_diff)
            self.hidden_error = np.multiply(self.hidden_error, h_diffsum)
            '''
            self.hidden_error = [self.hidden_s[l]._calculate_error_(self.hidden_nodes[l], self.output_mw[:,l], self.output_error) for l in range(self.num)]
            ...
            for j in range(self.num):
                self.o_change[:,j+1] = np.add(self.lr * np.multiply(self.output_error, self.hidden_nodes[j]), self.alpha * self.o_change_t[:,j])
            for i in range(784):
                self.h_change[:,i+1] = np.add(self.lr * np.multiply(self.hidden_error, x_i[i]), self.alpha * self.h_change_t[:,i])
            '''
